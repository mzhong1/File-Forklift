...
/*
    init_node_list: &str -> ForkliftResult<Vec<String>>
    REQUIRES: filename is a valid File where each line is the ip:port of a node
    ENSURES: returns the String vector of ip:port addresses wrapped in ForkliftResult,
    or returns an Error (IO error)
*/
fn init_node_list(filename: &str) -> ForkliftResult<Vec<String>> {
...
/*
    get_full_address: &str * &mut Vec<String> -> String
    REQUIRES: ip a valid ip address, node_names is not empty
    ENSURES: returns the ip:port associated with the input ip address
    that is stored in 
*/
fn get_full_address(ip: &str, node_names: &mut Vec<String>) -> ForkliftResult<String> {
    for n in node_names {
        if n.contains(ip) {
            return Ok(n.to_string());
        }
    }
    Err(ForkliftError::AddressNotFoundError)
}
...
/*
    ...
    return ""
*/
fn get_port_from_list(s: &str, nodes: &mut Vec<String>) -> String {
    let full_address = get_full_address(s, nodes).unwrap();
    get_port_from_address(s, &*full_address)
}
...
fn make_and_add_node(
    ...
    if !node_names.contains(&sent_address.to_string()) {
...
fn init_router(full_address: &str, node_names: &mut Vec<String>) -> ForkliftResult<Socket> {
    let mut router = Socket::new(Protocol::Bus)?;
    let current_port = get_port_from_list(&full_address, node_names);
...
fn send_getlist(
    ...
    let c_time = current_time_in_millis(SystemTime::now()).unwrap();
...
fn send_and_tickdown(
    ...
    if request.get_fds()[0].can_write() {
        ...
        if c_time > *heartbeat_at {
            ...
            tickdown_nodes(nodes, node_list);
            *heartbeat_at = c_time + interval
        }
    }
}
...
fn read_and_heartbeat(
    ...
    if request.get_fds()[0].can_read() {
        ...
        if msgtype == MessageType::HEARTBEAT {
            heartbeat_heard(&msg_body, node_names, nodes, liveness, router, &ip_address);
        }
...
fn heartbeat_loop(
    ...
    loop {
        ...
        Socket::poll(&mut request, interval as isize)?;
        debug!("Poll can read: {:?}", request.get_fds()[0].can_read());
        debug!("Poll can write: {:?}", request.get_fds()[0].can_write());

        read_and_heartbeat(&request, router, node_names, nodes, liveness, full_address);
...
fn main() -> ForkliftResult<()> {
    ...
    if joined.len() > 0 {
        ...
        node_names = init_node_list(filename)?;
    }
    ...
    let full_address = get_full_address(&ip_address, &mut node_names).unwrap();
