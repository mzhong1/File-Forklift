...
impl Entry {
    pub fn new(epath: &Path, context: &NetworkContext) -> Self {
        let (metadata, is_link) = match context.stat(epath) {
            Ok(stat) => (
                Some(stat),
...
///
/// make a directory endpoint in the destination filesytem, keeping Dos or Unix permissions
/// (depending on if the context is Samba or NFS)
///
/// @param path     the path of the directory being created
///
/// @param srcpath  the path of the equivalent directory from the source filesystem
///
...
pub fn make_dir(
    ...
    if !exist(&path, destfs) {
        match destfs.mkdir(path) {
            Ok(_) => (),
            Err(e) => {
        ...
        let (src_entry, dest_entry) = (Entry::new(&srcpath, fs), Entry::new(&path, destfs));
        // make sure permissions match
        match copy_permissions(&src_entry, &fs, &dest_entry, &destfs) {
            Ok(_) => debug!("Copy permissions successful"),
            Err(e) => {
                return Err(e);
            }
        };
...
pub fn make_dir_all(
    ...
    while !stack.is_empty() {
        ...
        match make_dir(&path, srcpath, fs, destfs) {
...
fn read_link(path: &Path, size: i64, context: &Nfs) -> ForkliftResult<String> {
    ...
    match context.readlink(path, &mut src_target) {
        Ok(_) => (),
        Err(e) => {
...
///                 need to write again
///
fn write_file(path: &Path, file: &FileType, buffer: &Vec<u8>, offset: u64) -> ForkliftResult<i32> {
...
pub fn checksum_copy(
    ...
    while { !end } {
        ...
        let src_buf = read_chunk(&src_file, offset, src_path)?;
        ...
        //update offset and check if num_written > 0
        if num_written > 0 {
...
///                     functions fail.
///
fn map_temp_acl(sid: &str, dest_ctx: &Smbc, dest_path: &PathBuf) -> ForkliftResult<Sid> {
...
///                     the mapping fail
///
fn map_name(sid: &str, dest_ctx: &Smbc, dest_path: &PathBuf) -> ForkliftResult<Sid> {
...
///
pub fn get_acl_list(fs: &Smbc, path: &PathBuf, plus: bool) -> ForkliftResult<Vec<SmbcAclValue>> {
...
///                 details)
///
fn change_stat_mode(context: &NetworkContext, path: &PathBuf, mode: u32) -> ForkliftResult<()> {
